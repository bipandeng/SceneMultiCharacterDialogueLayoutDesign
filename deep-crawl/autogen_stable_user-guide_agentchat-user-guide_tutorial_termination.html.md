...
在上一节中,我们探讨了如何定义智能体(Agent),并将它们组织成团队以解决任务。然而,一次运行可能会无限进行下去,在许多情况下,我们需要知道该在*何时*停止它们。这正是终止条件(Termination Condition)的作用。

AgentChat 通过提供一个基础的 [`TerminationCondition`](../../../reference/python/autogen_agentchat.base.html#autogen_agentchat.base.TerminationCondition "autogen_agentchat.base.TerminationCondition") 类以及若干继承自该类的实现,来支持多种终止条件。

终止条件是一个可调用对象(callable),它接收自上一次调用该条件*以来*的一系列 [`BaseAgentEvent`](../../../reference/python/autogen_agentchat.messages.html#autogen_agentchat.messages.BaseAgentEvent "autogen_agentchat.messages.BaseAgentEvent") 或 [`BaseChatMessage`](../../../reference/python/autogen_agentchat.messages.html#autogen_agentchat.messages.BaseChatMessage "autogen_agentchat.messages.BaseChatMessage") 对象。如果对话应当终止,则返回一个 [`StopMessage`](../../../reference/python/autogen_agentchat.messages.html#autogen_agentchat.messages.StopMessage "autogen_agentchat.messages.StopMessage");否则返回 `None`。一旦某个终止条件被触发,必须通过调用 [`reset()`](../../../reference/python/autogen_agentchat.base.html#autogen_agentchat.base.TerminationCondition.reset "autogen_agentchat.base.TerminationCondition.reset") 进行重置,然后才能再次使用。

关于终止条件,以下几点值得注意:

  * 它们是有状态的,但在每次运行([`run()`](../../../reference/python/autogen_agentchat.base.html#autogen_agentchat.base.TaskRunner.run "autogen_agentchat.base.TaskRunner.run") 或 [`run_stream()`](../../../reference/python/autogen_agentchat.base.html#autogen_agentchat.base.TaskRunner.run_stream "autogen_agentchat.base.TaskRunner.run_stream"))结束后会自动重置。

  * 它们可以使用 AND 和 OR 运算符进行组合。

Note

对于群聊团队(即 [`RoundRobinGroupChat`](../../../reference/python/autogen_agentchat.teams.html#autogen_agentchat.teams.RoundRobinGroupChat "autogen_agentchat.teams.RoundRobinGroupChat")、[`SelectorGroupChat`](../../../reference/python/autogen_agentchat.teams.html#autogen_agentchat.teams.SelectorGroupChat "autogen_agentchat.teams.SelectorGroupChat") 和 [`Swarm`](../../../reference/python/autogen_agentchat.teams.html#autogen_agentchat.teams.Swarm "autogen_agentchat.teams.Swarm")),终止条件会在每个智能体响应后被调用。虽然一次响应可能包含多条内部消息,但团队只会针对来自单次响应的所有消息调用一次终止条件。因此,该条件接收的是自上一次调用以来的"增量消息序列"(delta sequence)。

内置的终止条件包括:

  1. [`MaxMessageTermination`](../../../reference/python/autogen_agentchat.conditions.html#autogen_agentchat.conditions.MaxMessageTermination "autogen_agentchat.conditions.MaxMessageTermination"):在生成了指定数量的消息后停止,包括智能体消息和任务消息。

  2. [`TextMentionTermination`](../../../reference/python/autogen_agentchat.conditions.html#autogen_agentchat.conditions.TextMentionTermination "autogen_agentchat.conditions.TextMentionTermination"):当消息中提及了特定的文本或字符串(例如 "TERMINATE")时停止。

  3. [`TokenUsageTermination`](../../../reference/python/autogen_agentchat.conditions.html#autogen_agentchat.conditions.TokenUsageTermination "autogen_agentchat.conditions.TokenUsageTermination"):当使用了指定数量的提示(prompt)或补全(completion)令牌(token)时停止。这要求智能体在其消息中报告令牌使用情况。

  4. [`TimeoutTermination`](../../../reference/python/autogen_agentchat.conditions.html#autogen_agentchat.conditions.TimeoutTermination "autogen_agentchat.conditions.TimeoutTermination"):在指定的秒数之后停止。

  5. [`HandoffTermination`](../../../reference/python/autogen_agentchat.conditions.html#autogen_agentchat.conditions.HandoffTermination "autogen_agentchat.conditions.HandoffTermination"):当请求向特定目标交接(handoff)时停止。交接消息可用于构建诸如 [`Swarm`](../../../reference/python/autogen_agentchat.teams.html#autogen_agentchat.teams.Swarm "autogen_agentchat.teams.Swarm") 这样的模式。当你希望暂停运行并允许应用程序或用户在智能体向他们交接时提供输入,这非常有用。

  6. [`SourceMatchTermination`](../../../reference/python/autogen_agentchat.conditions.html#autogen_agentchat.conditions.SourceMatchTermination "autogen_agentchat.conditions.SourceMatchTermination"):在特定智能体响应后停止。

  7. [`ExternalTermination`](../../../reference/python/autogen_agentchat.conditions.html#autogen_agentchat.conditions.ExternalTermination "autogen_agentchat.conditions.ExternalTermination"):允许从运行外部以编程方式控制终止。这对于 UI 集成(例如聊天界面中的"停止"按钮)非常有用。

  8. [`StopMessageTermination`](../../../reference/python/autogen_agentchat.conditions.html#autogen_agentchat.conditions.StopMessageTermination "autogen_agentchat.conditions.StopMessageTermination"):当智能体产生 [`StopMessage`](../../../reference/python/autogen_agentchat.messages.html#autogen_agentchat.messages.StopMessage "autogen_agentchat.messages.StopMessage") 时停止。

  9. [`TextMessageTermination`](../../../reference/python/autogen_agentchat.conditions.html#autogen_agentchat.conditions.TextMessageTermination "autogen_agentchat.conditions.TextMessageTermination"):当智能体产生 [`TextMessage`](../../../reference/python/autogen_agentchat.messages.html#autogen_agentchat.messages.TextMessage "autogen_agentchat.messages.TextMessage") 时停止。

  10. [`FunctionCallTermination`](../../../reference/python/autogen_agentchat.conditions.html#autogen_agentchat.conditions.FunctionCallTermination "autogen_agentchat.conditions.FunctionCallTermination"):当智能体产生包含名称匹配的 [`FunctionExecutionResult`](../../../reference/python/autogen_core.models.html#autogen_core.models.FunctionExecutionResult "autogen_core.models.FunctionExecutionResult") 的 [`ToolCallExecutionEvent`](../../../reference/python/autogen_agentchat.messages.html#autogen_agentchat.messages.ToolCallExecutionEvent "autogen_agentchat.messages.ToolCallExecutionEvent") 时停止。

  11. [`FunctionalTermination`](../../../reference/python/autogen_agentchat.conditions.html#autogen_agentchat.conditions.FunctionalTermination "autogen_agentchat.conditions.FunctionalTermination"):当函数表达式在最后一次增量消息序列上求值为 `True` 时停止。这对于快速创建内置终止条件未涵盖的自定义终止条件非常有用。

## 基本用法#

为了演示终止条件的特性,我们将创建一个由两个智能体组成的团队:一个负责文本生成的主要智能体,以及一个对生成的文本进行审阅和提供反馈的评审智能体。
...
让我们来探索一下终止条件如何在每次 `run` 或 `run_stream` 调用之后自动重置,从而使团队能够从上次中断的地方继续对话。
...
对话在达到最大消息数限制后停止了。由于主要智能体还没有机会对反馈作出回应,让我们继续对话。
...
团队从上次的断点处继续,使得主要智能体能够对反馈作出回应。

## 组合终止条件#

下面我们来展示如何通过使用 AND (`&`) 和 OR (`|`) 运算符来组合终止条件,从而构造更复杂的终止逻辑。例如,我们将创建一个团队,当生成 10 条消息后,或当评审智能体批准消息时,团队会停止。
...
对话在评审智能体批准消息后停止,虽然它原本也可以在生成 10 条消息后停止。

另一种情况是,如果我们希望仅当两个条件都满足时才停止运行,则可以使用 AND (`&`) 运算符。
...
对于大多数使用场景而言,内置的终止条件已经足够。但是,在某些情况下,你可能需要实现一个不适合现有终止条件的自定义终止条件。你可以通过继承 [`TerminationCondition`](../../../reference/python/autogen_agentchat.base.html#autogen_agentchat.base.TerminationCondition "autogen_agentchat.base.TerminationCondition") 类来实现这一点。

在本例中,我们创建一个自定义终止条件,用于在特定函数被调用时停止对话。
...
让我们使用这个新的终止条件,在评审智能体通过调用 `approve` 函数来批准消息时停止对话。

首先,我们创建一个简单的函数,它将在评审智能体批准消息时被调用。
...
你可以看到,对话在评审智能体通过 `approve` 函数调用批准消息时停止。

__On this page

[ __Edit on GitHub](https://github.com/microsoft/autogen/edit/main/python/docs/src/user-guide/agentchat-user-guide/tutorial/termination.ipynb)

[ __Show Source](../../../_sources/user-guide/agentchat-user-guide/tutorial/termination.ipynb.txt)